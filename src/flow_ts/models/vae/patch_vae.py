...
class Encoder(nn.Module):
    ...
    def __init__(self, input_dim: int, latent_dim: int, patch_len: int) -> None:
        super().__init__()
        patch_dim = patch_len * input_dim  # dimension of each patch/token
    ...
    def forward(self, x: Tensor) -> tuple[Tensor, Tensor]:
        ...
        h = h + self.mlp(self.norm(h))  # local refinement
...
class Decoder(nn.Module):
    ...
    def __init__(self, latent_dim: int, output_dim: int, patch_len: int) -> None:
        super().__init__()
        patch_dim = patch_len * output_dim  # dimension of each patch/token

        # Initial linear projection
        self.proj = nn.Linear(latent_dim, patch_dim)
        ...
        self.mlp = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(2 * patch_dim, patch_dim),
        )
...
class PatchVAE(VAE):
    ...
    def __init__(self, input_dim: int, latent_dim: int, patch_len: int) -> None:
        ...
        self.encoder = Encoder(input_dim=input_dim, latent_dim=latent_dim, patch_len=patch_len)
        self.decoder = Decoder(latent_dim=latent_dim, output_dim=input_dim, patch_len=patch_len)
